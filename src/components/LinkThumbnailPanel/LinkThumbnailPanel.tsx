...
export const LinkThumbnailPanel: React.FC<LinkThumbnailPanelProps> = ({linkId}) => {
...
const onDelete = () => {
  ...
  return (
    <div style={{padding: '8px', display: 'flex', alignItems: 'center', flexDirection: 'column'}}>
      <Typography variant='body1'>Upload Link thumbnail</Typography>
      <Box
      onClick={handleClick}
      style={{
        display: "flex",
    ...
    >
      ...
      />
      <div>
      {!!localImagePath.length && <Button disabled={!localImagePath.length} onClick={onDelete} fullWidth variant='contained'>Remove</Button>}
      </div>
